interface ValidatorParams {
  ...
  const validator: (params: ValidatorParams) => void = ({ inputText, autoComplete, setErrorInputValue }) => {
    if (autoComplete === 'email') {
      ...
      const isValidName = inputText.length >= minLength && !disallowedCharacters.test(inputText);
    ...
    else if (autoComplete === 'street-address') {
